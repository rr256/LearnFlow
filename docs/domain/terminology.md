...
| Upload; attach a file; fetch a page; a resource's contents | Register a resource; where the material is | A *learning resource* is a record of **where material is**, never the material: nothing is uploaded, downloaded, fetched, extracted, or indexed. Wording that implies LearnFlow holds a copy of somebody's **file or web page** describes a capability that does not exist. **Narrowed by [ADR-037](../adr/ADR-037-learner-written-resource-notes.md)**: a *resource note* is content LearnFlow does hold, because the learner **typed it themselves**. That is not a file, it did not come off the web, and it names no location on their machine, so all three things this row was written to keep out stay out. Say a learner *wrote* or *kept* a note; never that they uploaded, attached, or imported one. |
