...
- Immediate objective: complete [Milestone 3](roadmap/milestones.md#milestone-3-planning-and-revision)'s remaining planning scope, then its revision half. What is open in planning is a **monthly** view; a generated **`daily`** plan, whose contents are deliberately still undecided and which the `/plan/today` view above is not; and **saying when a saved week cannot reach the horizon**, which is FR-004's third criterion and the only one of its three still unmet — learner postponement, the last part of its first, has since been delivered. Revision is unbuilt entirely: `revision_records` does not exist. Earlier milestones keep the open items their own checklists record — the `chromadb` Compose service, material status and study activity, and the progress overview among them — and each affected area still replaces relevant placeholders with approved project decisions before it is implemented.
