...
- Immediate objective: complete [Milestone 3](roadmap/milestones.md#milestone-3-planning-and-revision)'s remaining planning scope, then its revision half. What is open in planning is a generated **`monthly`** or **`daily`** plan — both approved `plan_type` values whose contents are deliberately still undecided, and which the `/plan/month` and `/plan/today` views above are not — and **revision**, which is unbuilt entirely. Saying when a saved week cannot reach the horizon — FR-004's third criterion — has since been delivered by PLN-006, so **FR-004 is met in full**; the monthly view closed FR-003's second criterion before it. Revision is unbuilt entirely: `revision_records` does not exist. Earlier milestones keep the open items their own checklists record — the `chromadb` Compose service, material status and study activity, and the progress overview among them — and each affected area still replaces relevant placeholders with approved project decisions before it is implemented.
