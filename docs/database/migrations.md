...
| `20260820_01` | `add_resource_note_search_index` — one GIN index over `to_tsvector('english', title \|\| ' ' \|\| body)` on `resource_notes`, so RES-013 can find the passages of a learner's own notes that mention a topic. **Creates one index and nothing else**: no table, no column, no constraint, and **no row is read, rewritten, or reinterpreted**. It is an index rather than a stored `tsvector` column deliberately — a column would alter a learner-owned table to hold a derived representation of note text, which [ADR-037](../adr/ADR-037-learner-written-resource-notes.md) kept out of the schema. The two-argument `to_tsvector` with a literal configuration is required, because the one-argument form is `STABLE` rather than `IMMUTABLE` and cannot be indexed. **No extension is installed**: `english` is built in, so neither `pg_trgm` nor `unaccent` is needed. **Its downgrade loses nothing a learner wrote** — an index is derived structure over text that stays in the table, unlike `20260819_01`, whose downgrade discards the notes themselves; searching falls back to a sequential scan. See [ADR-038](../adr/ADR-038-local-topic-note-retrieval.md). |
| `20260821_01` | `create_resource_files_table` — `resource_files`, holding one row per PDF a learner has uploaded against a piece of catalogued material. Creates one empty table and the index [schema.md](schema.md#resource_files) names, and **alters nothing**: no existing table, column, or constraint changes, and no stored row is read, rewritten, or reinterpreted. **The file bytes are not in the database.** They live in a Docker named volume mounted into the backend alone; `storage_key` is the opaque reference that finds them, and a `bytea` column was rejected because binaries bloat every dump. This is the **third addition beyond the tables [schema.md](schema.md) approves**, after `questions.author_learner_id` and `resource_notes`, and the second whole table: the approved shape puts `storage_key` and `metadata` on `resources` itself, which models **one** file per resource, and a learner keeps several — so those two columns stay uncreated rather than being half-built into a shape they cannot support. **Its downgrade drops the table and leaves the bytes in the volume**, orphaning them rather than deleting a learner's files to undo a schema change; re-applying the upgrade creates an empty table, so the rows do not come back. See [ADR-040](../adr/ADR-040-learner-uploaded-resource-files.md). |
...
- [ADR-038: Retrieve passages from a learner's own notes locally, when they ask](../adr/ADR-038-local-topic-note-retrieval.md) — the decision behind `20260820_01`, and why its downgrade loses nothing
