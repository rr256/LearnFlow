...
### Curriculum area, reviewed 2026-07-31

Reviewed against the mapping strategy and the supported constraints, which produced the conventions
and constraint decisions recorded above and in
[ADR-011](../adr/ADR-011-sqlalchemy-persistence-implementation.md).

Two review inputs did not exist yet and were therefore not covered: the **GATE CSE seed structure**
and the **first API contracts**, both of which arrive later in Milestone 1. If either turns out to
need a different shape — a subject code longer than the mapped 64 characters, say, or an ordering
rule the `position` uniqueness forbids — that is a follow-up migration, not an edit to the applied
one. The revision-scheduling rules are not relevant to this area.
