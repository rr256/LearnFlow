...
| `progress/StudyProgressOverview.tsx` | The progress overview's six panels — what each active plan covers, today's work, whether the saved week reaches the horizon, what the learner has marked, the learning stages they recorded by subject, and the reviews ready now — with its CSS Module. It renders `PlanFeasibility` and `LearningStagesBySubject` unchanged and reuses `planner/plan.ts` and `planner/today.ts` rather than copying them. It carries **no control of any kind** and writes nothing: each panel names where its action lives and links to it. **It counts nothing of its own** — the only figures are ones the API reported. Contracted by [ADR-029](../adr/ADR-029-progress-overview.md), and its stages panel by [ADR-030](../adr/ADR-030-learning-stages-by-subject-panel.md). |
