...
repository — and `owner_learner_id` stays nullable for curated content nothing writes yet. **A resource is never deleted**: RES-005 is unimplemented, and a learner puts material aside with `status: archived`, reversibly. *(Narrowed by RES-018 and RES-019 below for the **file** and the **note** kept against a resource — never the resource itself.)* **Nothing is recommended, ranked, or counted**: a topic's material is what the learner
...
PRG-004, which refuses a stage on one. **Writing lives on `/resources` alone** — add, edit, archive, and remove — while the curriculum view,
